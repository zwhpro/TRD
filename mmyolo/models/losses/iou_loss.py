...
def bbox_overlaps(pred: torch.Tensor,
    ...
    Returns:
        Tensor: shape (n, ).
    """
    assert iou_mode in ('iou', 'ciou', 'giou', 'siou','smoothGD',"wd","kl")
    ...
    if iou_mode == 'iou':
        ...
        union_x1_min, union_y1_min = torch.min(bbox1_x1, bbox2_x1), torch.min(bbox1_y1, bbox2_y1)
        union_x2_max, union_y2_max = torch.max(bbox1_x2, bbox2_x2), torch.max(bbox1_y2, bbox2_y2)

        # 计算bbox1和bbox2的外接框的对角线长度
        diagonal = (union_x2_max - union_x1_min) ** 2 + (union_y2_max - union_y1_min) ** 2

        # 计算bbox1和bbox2的中心点之间的距离
        distance = (cx1 - cx2) ** 2 + (cy1 - cy2) ** 2

        smoothGD = (1 / (1 + kl) - distance / diagonal + 1) / 2
        # smoothGD = 1 / (1 + kl) - distance / diagonal
        ious = smoothGD
    elif iou_mode == 'wd':
...
class IoULoss(nn.Module):
    ...
    def __init__(self,
        ...
        assert iou_mode in ('iou','ciou', 'siou', 'giou','smoothGD',"wd","kl")
